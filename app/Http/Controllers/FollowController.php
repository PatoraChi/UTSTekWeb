...
{
    public function toggleFollow(Request $request, User $profile)
    {
        if (!Session::has('user')) {
        ...
        $isFollowing = $follower->isFollowing($profile);
        
        if ($isFollowing) {
            // --- UNFOLLOW ---
            $follower->following()->detach($profile->id);
            $isFollowing = false;
        ...
        $newCount = $profile->followers()->count();
